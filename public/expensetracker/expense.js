...
function downloadExpense(){
    ...
    axios.get('http://51.20.7.24:3000/user/download', { headers: {"Authorization" : token} })
    .then((response) => {
...
function downloadedExpense() {
    ...
    const token = localStorage.getItem('jwtToken')
    axios.get('http://51.20.7.24:3000/user/downloads', { headers: {"Authorization" : token} })
    .then((res) => {
...
async function leaderboard(){
    ...
    await axios.get('http://51.20.7.24:3000/premium/leaderboard')
    .then(res => {
...
document.getElementById('buy-button').onclick = async function (e){
    const token = localStorage.getItem('jwtToken')
    //console.log(token)
    const response = await axios.post('http://51.20.7.24:3000/purchase/purchase-premium', {}, {headers: {'Authorization': token}
});
    console.log(response)
    ...
    var options = 
        {
            ...
            'handler': async function(response) {
                await axios.post('http://51.20.7.24:3000/purchase/updatetransactionstatus',
                {
    ...
    rzp1.on('payment.failed', async function (response) {
        await axios.post('http://51.20.7.24:3000/purchase/purchase-failure', {response}, {headers: {'Authorization': token}});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    expenseForm.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            const token = localStorage.getItem('jwtToken')
            //console.log(token)
            const response = await axios.post(`http://51.20.7.24:3000/expense/add-expense`, { amount, description, category},{headers :{'Authorization': token}});
            console.log(response)
    ...
    async function fetchAndDisplayExpenses(page,itemno) {
        try {
            ...
            const token = localStorage.getItem('jwtToken')
            //console.log(token)
            const response = await axios.get(`http://51.20.7.24:3000/expense/get-expense?page=${page}&pageitems=${itemno}`,{headers: {'Authorization': token }});
    ...
    async function deleteExpense(expenseId, expenseItem) {
        try {
            const token = localStorage.getItem('jwtToken')
            const response = await axios.delete(`http://51.20.7.24:3000/expense/delete-expense/${expenseId}`,{headers: {'Authorization': token}});
